  ...
  <div class="min-vh-100 bg-light d-flex align-items-center justify-content-center p-4">
    <div class="card border-0 shadow-sm p-4 w-100" style="max-width: 400px; border-radius: 1rem;">
      ...
      <div class="text-center mb-4">
        <h1 class="h4 fw-bold text-dark">Patient Login</h1>
      </div>
      ...
      <div class="d-grid gap-3">
        <input v-model="username" placeholder="Username" class="form-control p-3" />
        <input v-model="password" type="password" placeholder="Password" class="form-control p-3" />
      ...
      <div class="mt-4 pt-4 border-top text-center d-grid gap-3">
        <p class="small text-muted mb-0">
          Don't have an account? 
          <RouterLink to="/register" class="text-primary fw-bold text-decoration-none ms-1">Register</RouterLink>
        </p>
...
async function handleLogin() {
  ...
  try {
    await auth.loginPatient(username.value, password.value)
